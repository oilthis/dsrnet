...
class MASA(nn.Module):
    def __init__(self, args, teacher=''):
        ...
        if teacher=='True':
            self.mini = False
        if self.mini:
            n_blks = [2, 2, 2] # default: [4, 4, 4]
            n_blks_dec = [1, 1, 1, 6, 4, 2] # default: [2, 2, 2, 12, 8, 4]
        else:
            n_blks = [4, 4, 4]
        ...
        self.psize = 3
        self.lr_block_size = 16 # default: 8
        self.ref_down_block_size = 1.5
        self.dilations = [1] # default: [1, 2, 3]

        self.enc = Encoder(in_chl=in_chl, nf=nf, n_blks=n_blks)
        self.decoder = Decoder(nf, in_chl, n_blks=n_blks_dec)
        if self.depth:
            self.enc_d = Encoder(in_chl=1, nf=nf, n_blks=n_blks)
            self.decoder_d = Decoder(nf, 1, n_blks=n_blks_dec)

        if self.depth:
            self.mini_conv_1 = nn.Conv2d(128, 64, 3, 1, 1, bias=True)
            
            # self.fusion_block = nn.Sequential(
            #   nn.Conv2d(2*nf, nf, 3, 1, 1, bias=True),
    ...
    def search_dep(self, lr, reflr, lr_d, reflr_d, ks=3, pd=1, stride=1, dilations=[1, 2, 4]):
        # lr: [N, p*p, C, k_y, k_x]
        # reflr: [N, C, Hr, Wr]
        ...
        for i, dilation in enumerate(dilations):
            reflr_patches = F.unfold(reflr, kernel_size=(ks, ks), padding=dilation, stride=stride, dilation=dilation)  # [N, C*ks*ks, Hr*Wr]
            reflr_dep_patches = F.unfold(reflr_d, kernel_size=(ks, ks), padding=dilation, stride=stride, dilation=dilation)  # [N, C*ks*ks, Hr*Wr]
            lr_patches = lr[:, :, :, y - dilation: y + dilation + 1: dilation,
                                     x - dilation: x + dilation + 1: dilation]  # [N, p*p, C, ks, ks]
            lr_patches = lr_patches.contiguous().view(N, -1, C * ks * ks)  # [N, p*p, C*ks*ks]
            lr_patches = F.normalize(lr_patches, dim=2)
    ...
    def forward(self, lr, ref, ref_down, lr_dep=None, ref_dep_down=None, gt=None):
        _, _, h, w = lr.size()
        ...
        fea_lr_list = self.enc(lr)
        fea_reflr_list = self.enc(ref_down)
        fea_ref_l = self.enc(ref)
        if self.depth:
            fea_dep_lr_l = self.enc_d(lr_dep)
            fea_dep_ref_l = self.enc_d(ref_dep_down)
            fea_lr_l = [torch.cat([fea_lr_list[i], fea_dep_lr_l[i]], dim=1) for i in range(len(fea_lr_list))]
            fea_reflr_l = [torch.cat([fea_reflr_list[i], fea_dep_ref_l[i]], dim=1) for i in range(len(fea_reflr_list))]
        
            fea_lr_l = [self.mini_conv_1(f) for f in fea_lr_l]
            fea_reflr_l = [self.mini_conv_1(f) for f in fea_reflr_l]
        else:
            fea_lr_l = fea_lr_list
            fea_reflr_l = fea_reflr_list
        
        # print(fea_dep_lr_l[0].shape, fea_dep_lr_l[1].shape, fea_dep_lr_l[2].shape)
        # print(fea_dep_ref_l[0].shape, fea_dep_ref_l[1].shape, fea_dep_ref_l[2].shape)
        ...
        # print(fea_ref_l[0].shape, fea_ref_l[1].shape, fea_ref_l[2].shape)

        N, C, H, W = fea_lr_l[0].size()
        _, _, Hr, Wr = fea_reflr_l[0].size()

        lr_patches = F.pad(fea_lr_l[0], pad=(1, 1, 1, 1), mode='replicate')
        # lr_dep_patches = F.pad(fea_dep_lr_l[0], pad=(1, 1, 1, 1), mode='replicate')
        ...
        # lr_dep_patches = F.unfold(lr_dep_patches, kernel_size=(k_y + 2, k_x + 2), padding=(0, 0),
                            #   stride=(k_y, k_x))  # [N, C*(k_y+2)*(k_x+2), py*px]
        # print(lr_patches.shape)
        lr_patches = lr_patches.view(N, C, k_y + 2, k_x + 2, py * px).permute(0, 4, 1, 2, 3)  # [N, py*px, C, k_y+2, k_x+2]
        # lr_dep_patches = lr_dep_patches.view(N, C, k_y + 2, k_x + 2, py * px).permute(0, 4, 1, 2, 3)  # [N, py*px, C, k_y+2, k_x+2]
        # print('111')
        # print(lr_patches.shape)
        # print(fea_reflr_l[0].shape)
        # print(lr_dep_patches.shape)
        ## find the corresponding ref patch for each lr patch
        # if self.depth:
            # sorted_corr, ind_l = self.search_dep(lr_patches, fea_reflr_l[0], lr_dep_patches, fea_dep_ref_l[0],  
        ...
        # print(index, idx_x, idx_y, idx_x1, idx_x2, idx_y1, idx_y2)
        ...
        if gt is not None:
            ...
            if self.distill:
                return fea_lr_list, fea_dep_lr_l, out
            else:
                return out
