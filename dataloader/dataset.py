...
class TestSet2(Dataset):
    def __init__(self, args):
        ref_level = args.ref_level
        # ref_level = 5
        self.input_list = sorted(glob.glob(os.path.join(args.data_root, 'gt/*')))
        self.ref_list = sorted(glob.glob(os.path.join(args.data_root, 'lr/*')))
        self.scale = args.sr_scale
        self.transform = transforms.ToTensor()
...
class b100(Dataset):
    def __init__(self, args):
        self.input_list = sorted(glob.glob('dataset/b100/test/raw/input/*.png'))
        self.ref_folders = 'dataset/b100/test/raw/ref'
        self.scale = args.sr_scale
        self.transform = transforms.ToTensor()
    ...
    def __getitem__(self, idx):
        ...
        # ref_list = sorted(glob.glob(os.path.join(ref_folder, '*.png')))
        ref_list = [ref_folder]
...
class GuanDao(Dataset):
    def __init__(self, args):
        self.input_list = sorted([os.path.join(args.data_root, args.dataset, 'train/raw/input', name) for name in
                                  os.listdir(os.path.join(args.data_root, args.dataset, 'train/raw/input'))])
        self.ref_list = sorted([os.path.join(args.data_root, args.dataset, 'train/raw/ref', name) for name in
                                os.listdir(os.path.join(args.data_root, args.dataset, 'train/raw/ref'))])
        self.input_dep_list = sorted([os.path.join(args.data_root, args.dataset, 'train/depth/input', name) for name in
                                  os.listdir(os.path.join(args.data_root, args.dataset, 'train/depth/input'))])
        self.ref_dep_list = sorted([os.path.join(args.data_root, args.dataset, 'train/depth/ref', name) for name in
                                  os.listdir(os.path.join(args.data_root, args.dataset, 'train/depth/ref'))])
        self.scale = args.sr_scale
        self.data_augmentation = args.data_augmentation
        self.transform = transforms.ToTensor()
    ...
    def __getitem__(self, idx):
        if random.random() < 0.5:
            HR = cv2.imread(self.input_list[idx])
            Ref = cv2.imread(self.ref_list[idx])
            HR_dep = np.load(self.input_dep_list[idx])
            Ref_dep = np.load(self.ref_dep_list[idx])
        else:
            HR = cv2.imread(self.ref_list[idx])
            Ref = cv2.imread(self.input_list[idx])
            HR_dep = np.load(self.ref_dep_list[idx])
            Ref_dep = np.load(self.input_dep_list[idx])

        Ref = np.array(Image.fromarray(Ref).resize((448, 320), Image.BICUBIC))
        h, w, c = Ref.shape
        if h % 16 != 0 or w % 16 != 0:
            h_new = math.ceil(h / 16) * 16
            w_new = math.ceil(w / 16) * 16
        ...
        HR = np.array(Image.fromarray(HR).resize((448, 320), Image.BICUBIC))
        h, w, c = HR.shape
        if h % 16 != 0 or w % 16 != 0:
            h_new = math.ceil(h / 16) * 16
            w_new = math.ceil(w / 16) * 16
        ...
        # h, w, _ = HR.shape
        # HR = np.array(Image.fromarray(HR).resize((w // 2, h // 2), Image.BICUBIC))
        h, w, _ = HR.shape
        LR = np.array(Image.fromarray(HR).resize((w // self.scale, h // self.scale), Image.BICUBIC))
        ...
        HR_dep = HR_dep.squeeze()
        LR_dep = np.array(Image.fromarray(HR_dep).resize((w // self.scale, h // self.scale), Image.BICUBIC))

        Ref_dep = Ref_dep.squeeze()
        Ref_dep_down = np.array(Image.fromarray(Ref_dep).resize((w // self.scale, h // self.scale), Image.BICUBIC))

        # print(HR.shape)
        # print(LR.shape)
        # print(Ref.shape)
        # print(Ref_down.shape)
        # print(LR_dep.shape)
        # print(Ref_dep_down.shape)
        sample = {'HR': HR,
                  'LR': LR,
                  'Ref': Ref,
                  'Ref_down': Ref_down,
                  'LR_dep': LR_dep,
                  'Ref_dep_down': Ref_dep_down}

        for key in sample.keys():
            sample[key] = sample[key].astype(np.float32) / 255.
            if key == 'LR_dep' or key == 'Ref_dep_down':
                sample[key] = torch.from_numpy(sample[key]).unsqueeze(0).float()
            else:
                sample[key] = torch.from_numpy(sample[key]).permute(2, 0, 1).float()
...
class TestSet3(Dataset):
    def __init__(self, args):
        ...
        self.input_list = sorted(glob.glob(os.path.join(args.data_root, 'gt/*')))
        self.ref_list = sorted(glob.glob(os.path.join(args.data_root, 'lr/*')))
        self.input_dep_list = sorted(glob.glob(os.path.join(args.data_root, 'dep/*')))
        self.ref_dep_list = sorted(glob.glob(os.path.join(args.data_root, 'dep/*')))
        """
        self.input_list = sorted(glob.glob(os.path.join(args.data_root, 'GuanDao/result/hr/test/*')))
        self.ref_list = sorted(glob.glob(os.path.join(args.data_root, 'GuanDao/result/hr/test/*')))
        self.input_dep_list = sorted(glob.glob(os.path.join(args.data_root, 'GuanDao/test/depth/input/*')))
        self.ref_dep_list = sorted(glob.glob(os.path.join(args.data_root, 'GuanDao/test/depth/input/*')))
        """
        self.scale = args.sr_scale
        self.transform = transforms.ToTensor()
...
class TestGuanDao(Dataset):
    def __init__(self, args):
        ref_level = args.ref_level
        # ref_level = 5
        #  default
        self.input_list = sorted(glob.glob(os.path.join(args.data_root, 'GuanDao/test/raw/input/*')))
        self.ref_list = sorted(glob.glob(os.path.join(args.data_root, 'GuanDao/test/raw/ref/*')))
        self.input_dep_list = sorted(glob.glob(os.path.join(args.data_root, 'GuanDao/test/depth/input/*')))
        self.ref_dep_list = sorted(glob.glob(os.path.join(args.data_root, 'GuanDao/test/depth/ref/*')))
        """
        self.input_list = sorted(glob.glob(os.path.join(args.data_root, 'GuanDao/result/hr/test/*')))
        self.ref_list = sorted(glob.glob(os.path.join(args.data_root, 'GuanDao/result/hr/test/*')))
        self.input_dep_list = sorted(glob.glob(os.path.join(args.data_root, 'GuanDao/test/depth/input/*')))
