...
def main():
    ...
    parser.add_argument('--dataset', default='TestGuanDao', type=str, help='GuanDao')
    ...
    parser.add_argument('--testset', default='TestGuanDao', type=str, help='GuanDao')
    ...
    testset_ = getattr(importlib.import_module('dataset'), args.testset, None)
    test_dataset = testset_(args)
    test_dataloader = DataLoader(test_dataset, batch_size=1, num_workers=args.num_workers, shuffle=False)
    for j, batch_samples in enumerate(test_dataloader):
        print(j)
        for k in batch_samples:
            if not isinstance(batch_samples[k], list):
                print(k, batch_samples[k].shape)
            else:
                print(k, batch_samples[k])
        break
    ## test model
