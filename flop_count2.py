...
from models.archs.MASA_arch import *
...
def get_shape(val: object) -> typing.List[int]:
    ...
    if val.isCompleteTensor():  # pyre-ignore
        ...
        if not r:
            r = [1]
        return r
    elif val.type().kind() in ("IntType", "FloatType", "StringType"):
        return [1]
    else:
        raise ValueError()
...
def main():
    ...
    dep_lr = torch.FloatTensor(1, 1, h, w)
    dep_ref_down = torch.FloatTensor(1, 1, h, w)

    # if not args.depth:
    #     final_count = flop_count(net, inputs=(lr, ref, ref_down))
    # else:
    #     final_count = flop_count(net, inputs=(lr, ref, ref_down, dep_lr, dep_ref_down))
    final_count = flop_count(net, inputs=(lr,))
